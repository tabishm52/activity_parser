...
`lx_namespace_reset.tcx` reproduces a real TrainerRoad export quirk: a leaf reset to no namespace inside a correctly-namespaced `LX` wrapper, which should still resolve via the wrapper's namespace, alongside an unrelated no-namespace leaf that must stay unrecognized.
